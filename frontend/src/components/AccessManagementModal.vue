		...
		<div class="modal-box">
			...
			<div class="modal-header">
				<div class="modal-title-row">
					<button v-if="teamView !== 'list' && tab === 'teams'" class="btn-back" @click="goBackTeams"
						title="Back">
					...
					<h2 v-else-if="tab === 'teams' && teamView === 'team-users'">Users in <em>{{ selectedTeam?.name
							}}</em></h2>
					<h2 v-else-if="tab === 'teams' && teamView === 'team-dsns'">DSNs in <em>{{ selectedTeam?.name
							}}</em></h2>
						...
						<tbody>
							<tr v-for="team in teams" :key="team.id">
								...
								<td class="team-actions">
									<button class="btn btn-primary icon-btn" title="Manage Users"
										@click="openManageUsers(team)">👥</button>
									<button class="btn btn-primary icon-btn" :disabled="team.name === 'admin'"
										:title="team.name === 'admin' ? 'Admin team has access to all DSNs' : 'Manage DSNs'"
										@click="openManageDSNs(team)">🗄️</button>
									<button class="btn btn-danger icon-btn" :disabled="team.name === 'admin'"
										:title="team.name === 'admin' ? 'Cannot delete the admin team' : 'Delete team'"
										@click="handleDeleteTeam(team)">❌</button>
								</td>
							</tr>
				...
				<div v-else-if="teamView === 'team-users'" class="scroll-body">
					...
					<ul v-else class="member-list">
						<li v-for="user in allUsers" :key="user.id" class="member-item">
							<label>
								<input type="checkbox" :checked="teamUserIds.has(user.id)"
			...
			<template v-else-if="tab === 'users'">
				...
				<div v-if="showUserForm" class="user-form scroll-body">
					<div class="form-group">
						...
						<input v-model="userForm.username" type="text" placeholder="username" autocomplete="off"
							@keydown.enter="handleSaveUser" />
					</div>
					<div class="form-group">
						<label>Password</label>
						<input v-model="userForm.password" type="password" placeholder="password"
							autocomplete="new-password" @keydown.enter="handleSaveUser" />
					</div>
					<div class="form-group">
						<label>Teams</label>
					...
					<ul v-else class="member-list">
						<li v-for="user in users" :key="user.id" class="user-item">
							<div class="user-row">
								<div class="user-info">
									<span class="member-name">{{ user.username }}</span>
									<span v-for="t in (userTeams[user.id] ?? [])" :key="t.id" class="team-badge">{{
										t.name }}</span>
								</div>
								<div class="user-actions">
									<button class="btn btn-primary icon-btn" @click="revealJWTSecret(user.id)"
										:title="jwtSecretVisible[user.id] ? 'Hide JWT secret' : 'Show JWT secret'">🔑</button>
									<button class="btn btn-primary icon-btn" @click="openEditPassword(user.id)"
										title="Change password">📝</button>
									<button class="btn btn-danger icon-btn" :disabled="user.id === props.initAdminId"
										:title="user.id === props.initAdminId ? 'Cannot delete the initial admin user' : 'Delete user'"
										@click="handleDeleteUser(user)">❌</button>
								</div>
							</div>
							<!-- Inline password edit -->
							<div v-if="showEditPassword === user.id" class="inline-edit">
								<input v-model="editPassword" type="password" placeholder="New password"
									autocomplete="new-password" @keydown.enter="handleSavePassword(user.id)"
									@keydown.esc="cancelEditPassword" />
								<button class="btn btn-sm btn-primary" @click="handleSavePassword(user.id)">Save
									Password</button>
							...
							<div v-if="jwtSecretVisible[user.id]" class="jwt-reveal-panel">
								<div class="jwt-secret-row">
									...
									<button class="btn-copy-sm"
										@click="() => copyToClipboard(jwtSecretVisible[user.id])"
										title="Copy secret">Copy</button>
									<button class="btn-copy-sm btn-danger-sm" @click="refreshJWTSecret(user.id)"
										title="Generate new secret">Refresh</button>
								</div>
								<div class="jwt-mcp-header">
									<span class="jwt-mcp-label">Claude Desktop Config Example</span>
									<button class="btn-copy-sm"
										@click="() => copyToClipboard(mcpConfigExample(jwtSecretVisible[user.id]))"
										title="Copy config">Copy</button>
								</div>
